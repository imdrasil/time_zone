  ...
  struct Time
    struct Parser
      ...
      def time(zone : Zone?)
        ...
        time_zone = @zone || zone || Zone.default

        return time_zone.epoch(@epoch.not_nil!) if @epoch

        time_zone.new(@year, @month, @day, @hour, @minute, @second, nanosecond: @nanosecond)
      end
